...
int const MAX_SIZE = 64;
static pool_allocator *pool = NULL;
static binary_tree *tree = NULL;


void put(char *id, int value) { // Разделить на функции, пул создавать методом
    if (!pool) {
...
void putarr(char *id, int cell, int value) { // check index
    if (!pool) {
        pool = create_pool(MAX_SIZE);
        tree = create_binary_tree();
    }
...
int get(char *id) { // функции, функции, функции, результат и ошибки, передавать рпеременную с результатом по указателю
    if (!pool) {
