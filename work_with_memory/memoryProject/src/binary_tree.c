...
static void add_data(tnode *node, char *id, chunk *data) {
    node->data = data;
    node->string_data = id;
    node->pright = NULL;
    node->pleft = NULL;
...
static tnode* choose_node(tnode *node, char *id) {
    ...
    return NULL; // нашли элемент, не нужно добавлять
}
...
void add_tnode(binary_tree *tree, char *id, chunk *data) {
    ...
    if (!tmp) { // Вроде не надо, но на всякий случай
        return;
    }

    if (compare(id, tmp->string_data) == 1) {
